const GebridTransferManagerTest = artifacts.require("GebridTransferManagerTest.sol");
...
contract("GebridTransferManagerTest:doTransferTest()", accounts => {
  let RTM;
  let transferProxy;
  ...
  before(async () => {
    ...
    RTM = await GebridTransferManagerTest.new();
  ...
  describe("Check doTransfersExternal() with Royalties fees", () => {
    ...
    it("Transfer from ERC20 to ERC721(RoyaltiesV1 With Error), protocol fee 6% (buyerFee3%, sallerFee3%)", async () => {
      ...
      const erc721V1_Error = await ERC721_V1_Error.new("Gebrid", "GBRD", "https://ipfs.gebrid.com");
    ...
    it("Transfer from ERC1155(RoyaltiesV2 With Error) to ERC20, protocol fee 6% (buyerFee3%, sallerFee3%)", async () => {
      const erc1155V2_Error = await ERC1155_V2_Error.new("https://ipfs.gebrid.com");
