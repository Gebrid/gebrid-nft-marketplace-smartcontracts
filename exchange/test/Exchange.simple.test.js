...
const ExchangeSimple = artifacts.require("ExchangeSimple.sol");
const ExchangeSimple_1 = artifacts.require("ExchangeSimple_1.sol");
...
contract("ExchangeSimple", accounts => {
  let testing;
  let transferProxy;
  ...
  before(async () => {
    ...
    testing = await deployProxy(ExchangeSimple, [transferProxy.address, erc20TransferProxy.address], { initializer: "__ExchangeSimple_init" });
  });
  ...
  it("upgrade works", async () => {
    const wrapper = await ExchangeSimple_1.at(testing.address);
    await expectThrow(
      wrapper.getSomething()
    );

    await upgradeProxy(testing.address, ExchangeSimple_1);
  ...
  describe("validate", () => {
    it("should not let proceed if taker is not correct", async () => {
      ...
      await expectThrow(
        testing.matchOrders(left, leftSig, right, rightSig)
      );

      await expectThrow(
        testing.matchOrders(right, leftSig, left, rightSig)
      );
    });
