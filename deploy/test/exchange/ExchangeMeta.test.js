const ExchangeMeta = artifacts.require("ExchangeMeta.sol");
...
const GebridTestHelper = artifacts.require("GebridTestHelper.sol");
...
contract("exchange meta", accounts => {
  let exchangeMeta
  let transferProxy;
  ...
  before('before', async function () {
    ...
    helper = await GebridTestHelper.new()

    exchangeMeta = await ExchangeMeta.deployed();

    domainData = {
      name: "ExchangeMeta",
      version: "1",
      verifyingContract: exchangeMeta.address,
      salt: salt
    };
  ...
  describe("Check Methods", function () {
    ...
    it("Should be able to send transaction successfully, and check Event, that emit from method, execute as MetaTx", async () => {
      ...
      const nonce = await exchangeMeta.getNonce(publicKey);
      ...
      const resultExecMataTx = await exchangeMeta.executeMetaTransaction(publicKey, functionSignature, r, s, v, { from: accounts[1] });
      let hash;
    ...
    it("Exchange meta, orders can match", async () => {
      ...
      const tx = await exchangeMeta.matchOrders(left, "0x", right, await getSignature(right, accounts[1]), { from: accounts[2], value: 300 });
  ...
  async function getSignature(order, signer) {
    return sign(order, signer, exchangeMeta.address);
  }
