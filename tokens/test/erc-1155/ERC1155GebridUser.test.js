const Testing = artifacts.require("ERC1155Gebrid.sol");
const UpgradeableBeacon = artifacts.require("UpgradeableBeacon.sol");
const ERC1155GebridUserFactoryC2 = artifacts.require("ERC1155GebridFactoryC2.sol");
...
contract("ERC1155GebridUser", accounts => {
  ...
  beforeEach(async () => {
    ...
    await token.__ERC1155GebridUser_init(name, "TST", "ipfs:/", "ipfs:/", [], accounts[6], accounts[7], {from: tokenOwner});
  });
  ...
  it("mint and transfer by minter, token create by Factory", async () => {
    ...
    factory = await ERC1155GebridUserFactoryC2.new(beacon.address, ZERO, ZERO);
    const salt = 3;
    ...
    truffleAssert.eventEmitted(resultCreateToken2, 'Create1155GebridUserProxy', (ev) => {
        addrToken2 = ev.proxy;
      return true;
    });
    ...
    truffleAssert.eventEmitted(resultCreateToken3, 'Create1155GebridUserProxy', (ev) => {
      addrToken3 = ev.proxy;
    return true;
    });
