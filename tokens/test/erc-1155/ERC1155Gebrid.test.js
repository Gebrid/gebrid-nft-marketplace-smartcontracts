...
contract("ERC1155Gebrid", accounts => {
  ...
  beforeEach(async () => {
    ...
    await token.__ERC1155Gebrid_init(name, "TST", "ipfs:/", "ipfs:/", whiteListProxy, proxyLazy.address, {from: tokenOwner});
  });
...
//      event BurnLazy(address indexed operator, address indexed account, uint256 id, uint256 amount);
  ...
  describe("Mint and transfer  ()", () => {
    it("mint and transfer by minter, token create by Factory", async () => {
      ...
      factory = await ERC1155GebridFactoryC2.new(beacon.address, transferProxy.address, proxyLazy.address);
      const salt = 3;
      ...
      truffleAssert.eventEmitted(resultCreateToken2, 'Create1155GebridProxy', (ev) => {
          addrToken2 = ev.proxy;
        return true;
      });
      ...
      truffleAssert.eventEmitted(resultCreateToken3, 'Create1155GebridProxy', (ev) => {
        addrToken3 = ev.proxy;
      return true;
      });
